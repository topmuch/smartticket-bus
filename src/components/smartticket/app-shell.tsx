...
import GuichetRegional from './views/guichet-regional';
import { DeparturesManager } from './views/departures-manager';
...
import { StationDepartureBoard } from '@/components/display/station-departure-board';
...
const navByRole: Record<UserRole, NavItem[]> = {
  SUPERADMIN: [
    ...
    { id: 'departures', label: 'Départs', icon: <Clock className="w-4 h-4" /> },
    ...
    { id: 'departure-board', label: 'Tableau Départs', icon: <Monitor className="w-4 h-4" /> },
    { id: 'display', label: 'Affichage Gare', icon: <Monitor className="w-4 h-4" /> },
    { id: 'audit', label: 'Audit & Logs', icon: <FileText className="w-4 h-4" /> },
  ],
  OPERATOR: [
    { id: 'sell-regional', label: 'Ticket Voyage', icon: <Store className="w-4 h-4" /> },
